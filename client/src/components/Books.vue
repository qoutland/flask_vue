  ...
  <div class="container">
    <div class="row">
      <div class="col-sm-10">
        <h1>Books</h1>
        <hr /><br /><br />
        <alert :type="type" :message="message" v-if="showMessage"></alert>
        <button type="button" class="btn btn-success btm-sm" v-b-modal.book-modal>Add Book</button>
        <br /><br />
    ...
    <b-modal ref="addBookModal" id="book-modal" title="Add a new book" hide-footer>
      ...
      <b-form @submit="onSubmitUpdate" @reset="onReset" class="w-100">
        ...
        label-for="form-title-edit-input">
          <b-form-input id="form-title-input" type="text" v-model="editForm.title"
          required placeholder="Enter title">
          </b-form-input>
        </b-form-group>
        <b-form-group id="form-author-edit-group" label="Author:"
        label-for="form-author-edit-input">
          <b-form-input id="form-author-input" type="text" v-model="editForm.author"
  ...
  methods: {
    ...
    addBook(payload) {
      ...
      axios.post(path, payload)
        .then((res) => {
          this.getBooks();
          if (res.data.status === 'error') this.type = 'danger';
    ...
    updateBook(payload, bookID) {
      ...
      axios.put(path, payload)
        .then((res) => {
          this.getBooks();
          if (res.data.status === 'error') this.type = 'danger';
          if (res.data.status === 'success') this.type = 'warning';
          this.message = res.data.message;
          this.showMessage = true;
        })
        .catch((error) => {
